    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        if (request.getMethod().equalsIgnoreCase("POST") && request.getRequestURI().equals(LOGIN_PATH)) {
            ...
            if (contentType != null && contentType.contains("application/json")) {
                ...
                kakaoAccessToken = loginData.get("accessToken");
                kakaoRefreshToken = loginData.get("refreshToken");
            }
            // 폼 데이터 처리 (기존 코드)
            else {
                kakaoAccessToken = request.getParameter("accessToken");
                kakaoRefreshToken = request.getParameter("refreshToken");
            }
